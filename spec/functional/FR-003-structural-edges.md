...
- If an import specifier does not resolve to a path present in the batch, then
  the library SHALL omit the edge rather than emit an edge to a synthesized
  target, and SHALL record the unresolved specifier as a diagnostic.
- The library SHALL treat a package-absolute or bare specifier as unresolved
  under this requirement, since resolving it requires dependency metadata the
...
| FR-003-AC-3 | An import of a third-party package yields no edge and one unresolved-specifier diagnostic | Test (TC-017) |
