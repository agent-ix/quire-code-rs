...
collection uses a caller-supplied timestamp that is part of the pinned input
tuple rather than reading the clock.
...
| NFR-005-AC-3 | The versioned record contains no clock-derived timestamp, absolute path, host name, PID, or random value; its required timestamp is a pinned caller input. | Test (TC-128) |
